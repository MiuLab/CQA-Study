...
class BatchGen:
    ...
    def __iter__(self):
        ...
        for datum in data:
            ...
            if self.answer_span_in_context or self.opt.get('PREV_ANS_INDICATOR', 0) > 0:
                x_features = torch.Tensor(batch_size, x_len, 5).fill_(0)
            else:
                x_features = torch.Tensor(batch_size, x_len, 4).fill_(0)
            x_prev_ans_mask = torch.Tensor(batch_size, x_len, 1).fill_(1)
            ...
            for i in range(qa_len):
                ...
                for j in range(i - self.prev_ans, i + 1):
                    ...
                    q_char = [[0]] + datum['qas'][j]['annotated_question']['charid'][:self.ques_max_len - 1]
                    if j >= i - self.prev_ques:
                ...
                for j in range(i - self.opt['PREV_ANS_INDICATOR'], i):
                    if j < 0:
                        continue
                ...
                for j in range(i - self.opt['PREV_ANS_MASK'], i):
                    if j < 0:
                        continue
                    st = datum['qas'][j]['answer_span'][0]
                    ed = datum['qas'][j]['answer_span'][1] + 1
                    x_prev_ans_mask[i, st:ed, 0] = 0
            ...
            if self.use_cuda:
                ...
                x_prev_ans_mask = Variable(x_prev_ans_mask.cuda())
                ...
                x_prev_ans_mask = Variable(x_prev_ans_mask)
            ...
            query_bert, query_bert_mask, query_bert_offsets, ground_truth, context_str, context_words, context_word_offsets, answer_strs, context_id, turn_ids, x_prev_ans_mask)
