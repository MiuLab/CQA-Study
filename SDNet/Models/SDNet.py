...
class SDNet(nn.Module):
    ...
    '''
    def forward(self, x, x_single_mask, x_char, x_char_mask, x_features, x_pos, x_ent, x_bert, x_bert_mask, x_bert_offsets, q, q_mask, q_char, q_char_mask, q_bert, q_bert_mask, q_bert_offsets, context_len, x_prev_ans_mask):
        ...
        if self.opt.get('PREV_ANS_MASK', 0) > 0:
            if 'ANSWER_SPAN_IN_CONTEXT_FEATURE' in self.opt:
                x_input[..., :-1].masked_fill_(x_prev_ans_mask == 0, 0)
            else:
                x_input.masked_fill_(x_prev_ans_mask == 0, 0)
