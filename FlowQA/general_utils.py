...
def feature_gen(C_docs, Q_CID, Q_docs, no_match, no_position):
    ...
    for question, context_id in zip(Q_docs, Q_CID):
        ...
        if no_match:
            ...
            if no_position:
                qwords = [w.text for w in question]
                if '//' in qwords:
                    sep_index = qwords.index('//')
                    question = question[sep_index + 1:]
    ...
    def __iter__(self):
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            for first_QID in context_batch[5]:
                ...
                # shuffle by prob
                if random.random() < self.prob_shuffle:
                    random.shuffle(question_seq)
    ...
    def __iter__(self):
        ...
        for batch_i in range((self.context_num + self.batch_size - 1) // self.batch_size):
            ...
            for first_QID in context_batch[5]:
                ...
                # shuffle the order of turns in a dialog by probility
                if random.random() < self.prob_shuffle:
                    random.shuffle(question_seq)
