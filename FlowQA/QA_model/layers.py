...
class StackedBRNN(nn.Module):
    ...
    def forward(self, x, x_mask=None, return_list=False, additional_x=None, previous_hiddens=None, hidden_mask=None):
        ...
        for i in range(self.num_layers):
            ...
            if hidden_mask is None:
                ...
                rnn_output = []
                for t in range(rnn_input.shape[0]):
                    rnn_step_output, masked_hidden = \
                        self.rnns[i](rnn_input[t: t+1], masked_hidden)
                    masked_hidden.masked_fill_(hidden_mask[t: t+1], 0)
                    rnn_output.append(rnn_step_output)
                rnn_output = torch.cat(rnn_output, 0)
