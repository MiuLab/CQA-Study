...
class FlowQA(nn.Module):
    ...
    def forward(self, x1, x1_c, x1_f, x1_pos, x1_ner, x1_mask, x2_full, x2_c, x2_full_mask):
        ...
        if self.opt['mask_prev_ans']:
            assert not self.opt.get('use_dialog_act')
            indicator_dim = -2
            x1_input = x1_input * (1 - x1_f.view(-1, x1_f.size(-2), x1_f.size(-1))[:, :, indicator_dim:indicator_dim + 1])
            if self.opt['explicit_dialog_ctx'] == 2:
                indicator_dim = -4
                x1_input = x1_input * (1 - x1_f.view(-1, x1_f.size(-2), x1_f.size(-1))[:, :, indicator_dim:indicator_dim + 1])

            if 'remove_dim_indicator' in self.opt:
                x1_input = x1_input[:, :, :indicator_dim]
            else:
                x1_input = torch.cat([
                    x1_input[:, :, :indicator_dim],
                    x1_f[:, :, :, indicator_dim:].view(-1, x1_f.size(-2), -indicator_dim)
                ], -1)

        if self.opt.get('remove_indicator') \
           and self.opt['explicit_dialog_ctx'] > 0:
            indicator_dim = -2
            x1_input.data[:, :, indicator_dim] = 0
            if self.opt['explicit_dialog_ctx'] == 2:
                indicator_dim = -4
                x1_input.data[:, :, indicator_dim] = 0
        ...
        def flow_operation(cur_h, flow):
            ...
            if not self.opt['mask_prev_ans']:
                hidden_mask = None
            else:
                with torch.no_grad():
                    hidden_mask = x1_f.view(
                        -1, x1_f.size(-2), x1_f.size(-1)
                    )[:, :, -2:-1]
                    if self.opt['explicit_dialog_ctx'] == 2:
                        hidden_mask += x1_f.view(
                            -1, x1_f.size(-2), x1_f.size(-1)
                        )[:, :, -4:-3]
                    # [bs * max_qa_pair, ctx_len, dim_f]
                    hidden_mask = hidden_mask > 0
                    hidden_mask = hidden_mask \
                        .view(x1_f.size(0), x1_f.size(1), x1_f.size(2), 1) \
                        .transpose(0, 1).contiguous() \
                        .view(x1_f.size(1), x1_f.size(0) * x1_f.size(2), 1)

            flow_out = flow(flow_in, hidden_mask=hidden_mask)
        ...
        if self.opt['do_hierarchical_query']:
            if not self.opt['mask_prev_ans']:
                ...
                question_avg_hidden = self.hier_query_rnn(question_avg_hidden.view(x1_full.size(0) * x1_full.size(1), 1, -1))
