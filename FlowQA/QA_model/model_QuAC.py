...
class QAModel(object):
    ...
    def calc_grad(self, batch):
        # Train mode
        self.network.eval()

        # since "cudnn RNN backward can only be called in training mode"
        def fix_rnn_train_mode(m):
            if type(m) == nn.GRU or type(m) == nn.LSTM:
                m.train()
        self.network.apply(fix_rnn_train_mode)
        ...
        # Transfer to GPU
        if self.opt['cuda']:
            inputs = [e.cuda() for e in batch[:9]]
            overall_mask = batch[9].cuda()

            answer_s = batch[10].cuda()
            answer_e = batch[11].cuda()
            answer_c = batch[12].cuda()
        else:
            inputs = [e for e in batch[:9]]
            overall_mask = batch[9]
        ...
        # Clear gradients and run backward
        grads = torch.autograd.grad(loss, self.network.x1_input)
