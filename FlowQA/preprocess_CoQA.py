...
parser.add_argument('--dev_file', type=str, default='CoQA/dev.json')
parser.add_argument('--output_dir', type=str, default='CoQA/')
parser.add_argument('--no_prepend_answer', action='store_true')
...
def proc_train(ith, article):
    ...
    for j, (question, answers) in enumerate(zip(article['questions'], article['answers'])):
        ...
        if j > 0 and not args.no_prepend_answer:
...
# features
trC_tags, trC_ents, trC_features = feature_gen(trC_docs, train.context_idx, trQ_docs, args.no_match, args.no_position)
...
path = os.path.join(args.output_dir, 'train_meta.msgpack')
...
path = os.path.join(args.output_dir, 'train_data.msgpack')
with open(path, 'wb') as f:
...
# features
devC_tags, devC_ents, devC_features = feature_gen(devC_docs, dev.context_idx, devQ_docs, args.no_match, args.no_position)
...
path = os.path.join(args.output_dir, 'dev_meta.msgpack')
with open(path, 'wb') as f:
    msgpack.dump(meta, f)
...
path = os.path.join(args.output_dir, 'dev_data.msgpack')
with open(path, 'wb') as f:
    msgpack.dump(result, f)
